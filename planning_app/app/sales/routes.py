...
@sales_bp.route("/model-report")
@login_required
@permission_required("view_sales")
def model_report():
    models_selected = [v.strip() for v in request.args.getlist("model") if v.strip()]
    closed_months = request.args.get("closed_months", 12, type=int)
    if closed_months not in (3, 6, 12):
        closed_months = 12
    ...
    models = services.get_model_list()

    if not models_selected:
        overview = services.get_model_overview(closed_months)
        return render_template(
            "sales/model_report.html",
            title="Model Report",
            models=models,
            models_selected=[],
            closed_months=closed_months,
    ...
    data = services.get_model_report(models_selected, closed_months)
    if data is None:
        return render_template(
            "sales/model_report.html",
            title="Model Report",
    ...
    title = f"Model Report \u2013 {data['model_info']['model']}"
    return render_template(
        "sales/model_report.html",
        title=title,
        models=models,
