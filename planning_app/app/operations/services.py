'''Business logic for Operations reports and exports.'''
...
def get_wip_overview(args):
    ...
    shortage_group = args.get('shortage_group', '')  # '' | 'fabric' | 'component' | 'either' | 'no_shortage'
...
def get_wip_export(args):
    ...
    shortages_only = args.get('shortages_only', '0') == '1'
    shortage_group = args.get('shortage_group', '')
...
def get_quick_wins_export(args):
...
def get_daily_output(args):
    today         = date.today()
    date_7d       = today - timedelta(days=6)
    date_from_str = args.get('date_from', today.isoformat())
    date_to_str   = args.get('date_to',   today.isoformat())
    try:
