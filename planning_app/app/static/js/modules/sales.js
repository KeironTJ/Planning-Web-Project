 ...
 *
 * Handles:
 *   sales/customer_report.html — charts + TomSelect customer picker
 *   sales/model_report.html    — charts + TomSelect model picker
 * Reads:
 *   window.CUSTOMER_REPORT_DATA (set by template when a customer is selected)
 *   window.MODEL_REPORT_DATA    (set by template when a model is selected)
 */
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ── TomSelect on the customer / model pickers (always present) ────────
    var custSel = document.getElementById('customerSelect');
    if (custSel && typeof TomSelect !== 'undefined') {
        new TomSelect(custSel, {
    ...
    if (modelSel && typeof TomSelect !== 'undefined') {
        new TomSelect(modelSel, {
    ...
    if (window.CUSTOMER_REPORT_DATA)  renderCustomerReport(window.CUSTOMER_REPORT_DATA);
    if (window.MODEL_REPORT_DATA)     renderModelReport(window.MODEL_REPORT_DATA);
    if (window.MODEL_OVERVIEW_DATA)   renderModelOverview(window.MODEL_OVERVIEW_DATA);

    paginateTable('openOrdersTable',     'openOrdersNav',     10);
    paginateTable('closedOrdersTable',   'closedOrdersNav',   10);
    paginateTable('modelsOverviewTable', 'modelsOverviewNav', 25);
});

// ── Customer report charts ─────────────────────────────────────────────────
...
function renderModelReport(D) {
    ...
    // ── 5. Size mix horizontal bar ─────────────────────────────────────────
    if (D.bySize) {
        var sizeCtx = document.getElementById('chartSize');
        if (sizeCtx) {
            new Chart(sizeCtx, {
                type: 'bar',
                data: {
                    labels: D.bySize.map(function (r) { return r.size; }),
                    datasets: [{ label: 'Value (\u00a3)', data: D.bySize.map(function (r) { return r.value; }), backgroundColor: 'rgba(25,135,84,0.65)' }],
                },
                options: { indexAxis: 'y', responsive: true, maintainAspectRatio: true, plugins: { legend: { display: false } }, scales: { x: { ticks: { callback: fmtGbp } } } },
            });
        }
...
// ── Shared helper: bar (value) + line (cumulative %) Pareto chart ─────────
function buildParetoChart(canvasId, labels, values, barColor) {
    ...
    new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [
                { label: 'Value (\u00a3)', data: values, backgroundColor: barColor || 'rgba(13,202,240,0.65)', yAxisID: 'yVal', order: 2 },
                { label: 'Cumulative %', data: cumPct, type: 'line', borderColor: 'rgba(220,53,69,0.9)', borderWidth: 2, pointRadius: 3, fill: false, tension: 0.3, yAxisID: 'yPct', order: 1 },
            ],
        },
        options: {
...
// ── Model overview charts (no model selected — all-models landing page) ───
function renderModelOverview(D) {

    // ── 1. Forward demand bar + line (open book) ───────────────────────────
    var demandCtx = document.getElementById('chartDemand');
    if (demandCtx) {
        new Chart(demandCtx, {
            type: 'bar',
    ...
    // ── 2. Top models — open book Pareto ───────────────────────────────────
    if (D.topModelsOpen) {
        buildParetoChart(
            'chartModelsParetoOpen',
            D.topModelsOpen.map(function (r) { return r.model; }),
            D.topModelsOpen.map(function (r) { return r.value; }),
            'rgba(13,110,253,0.6)'
        );
    }

    // ── 3. Open orders — order type bar + line ─────────────────────────────
    if (D.byOrderType) {
        var otCtx = document.getElementById('chartOrderType');
        if (otCtx) {
            new Chart(otCtx, {
    ...
    // ── 4. Monthly intake bar + line (closed book) ─────────────────────────
    if (D.monthlyIntake) {
        var mCtx = document.getElementById('chartMonthly');
        if (mCtx) {
            new Chart(mCtx, {
    ...
    // ── 5. Top models — closed book Pareto ─────────────────────────────────
    if (D.topModelsClosed) {
        buildParetoChart(
            'chartModelsParetoClosed',
            D.topModelsClosed.map(function (r) { return r.model; }),
            D.topModelsClosed.map(function (r) { return r.value; }),
